...
[NOTE]
====
If you already have a Transit Gateway (TGW) or similar, you can skip the public subnet configuration
====

[NOTE]
====
When creating subnets, make sure that subnet(s) are created in availability zones that have ROSA instances types available. If AZ is not “forced”, the subnet is created in a random AZ in the region. Force AZ using the `--availability-zone` argument in the `create-subnet` command.
====
...
export PL_REGION=us-west-1 # Make sure to use a region where you haven't deployed a cluster yet
export PL_VERSION=4.13.4
...
I: Created role 'ManagedOpenShift-Installer-Role' with ARN 'arn:aws:iam::870651848115:role/ManagedOpenShift-Installer-Role'
I: Created role 'ManagedOpenShift-ControlPlane-Role' with ARN 'arn:aws:iam::870651848115:role/ManagedOpenShift-ControlPlane-Role'
I: Created role 'ManagedOpenShift-Worker-Role' with ARN 'arn:aws:iam::870651848115:role/ManagedOpenShift-Worker-Role'
I: Created role 'ManagedOpenShift-Support-Role' with ARN 'arn:aws:iam::870651848115:role/ManagedOpenShift-Support-Role'
...
[source,sh]
----
watch "rosa describe cluster --cluster ${PL_CLUSTER_NAME} | grep State"
----
+
.Sample Output
[source,texinfo]
...
.. Determine the AMI ID to use for your jump host and store it in PL_AMI_ID
+
[source,sh]
----
export PL_AMI_ID=$(aws ssm get-parameters \
  --region ${PL_REGION} \
...
[source,sh]
----
export IP_JUMPBOX=$(aws ec2 describe-instances \
  --region ${PL_REGION} \
...
[source,sh]
----
export PL_BASE_DOMAIN=pl-sts-szrxx.93vg.p1.openshiftapps.com
----
...
. SSH to your jumpbhost instance, tunneling traffic for the appropriate hostnames. Be sure to use your new/existing private key, the OpenShift DNS for $YOUR_OPENSHIFT_DNS and your jump host IP for $YOUR_EC2_IP
...
[source,sh]
----
aws ec2 delete-nat-gateway --nat-gateway-id ${PL_NAT} | jq .
aws ec2 release-address --allocation-id=${PL_EIP} | jq .
aws ec2 detach-internet-gateway --vpc-id ${PL_VPC_ID} --internet-gateway-id ${PL_IGW} | jq .
aws ec2 delete-subnet --subnet-id=${PL_PRIVATE_SUBNET} | jq .
PL_aws ec2 delete-subnet --subnet-id=${PL_PUBLIC_SUBNET} | jq .
aws ec2 delete-route-table --route-table-id=${PL_RTB} | jq .
aws ec2 delete-route-table --route-table-id=${PL_RTB_NAT} | jq .
aws ec2 delete-internet-gateway --internet-gateway-id ${PL_IGW} | jq .
aws ec2 delete-vpc --vpc-id{=$PL_VPC_ID} | jq .
----
