...
Therefore we need to label the Machine pool itself.
...
[source,sh,role=copy]
----
rosa edit machinepool -c ${CLUSTER_NAME} --labels tier=frontend Default
----
+
.Sample Output
...
Note the application is exposed over the default ingress using a predetermined URL and trusted TLS certificate.
