...
. Update the `${HOME}/${PL_CLUSTER_NAME}.ovpn` configuration file to add three lines of DNS information just above the line starting with `<ca>`. Make sure you use the actual values that you retrieved earlier.
+
[source,texinfo]
----
...
. Now you can log into the OpenShift cluster using the `oc login` command.
