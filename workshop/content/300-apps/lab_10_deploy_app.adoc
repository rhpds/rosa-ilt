...
image:../media/irsa-sts.jpeg[width=100%]
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "$(rosa describe cluster -c ${CLUSTER_NAME} -o json | jq -r .aws.sts.oidc_endpoint_url | sed -e 's/^https:\/\///'):sub": "system:serviceaccount:microsweeper-ex:microsweeper"
        }
      }
    }
  ]
